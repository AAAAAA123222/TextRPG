...
namespace TextRPG
{
    internal class Program
    {
        ...
        //가능하면 경험치도 상태창에 추가하기
        //레벨이 오를때마다 필요 경험치가 올라가게끔 할 수 있을 것 같지만 일단 보류(int reqExp 변수를 추가하고, 매 레벨업 함수가 실행될때마다 exp가 초기화된 다음 reqExp의 값을 올리기)
        ...
        //체력이 줄어드는 것까진 구현했는데 체력이 0 이하가 되었을 때 게임이 종료되는 기능은 딱히 명시되어 있지 않아서 보류(체력이 줄어드는 걸 메서드로 구현한 뒤 줄어들고 나서 체력이 0이 되었을 때 isAlive 불리언을 false로 바꾸는 등)
